	...
	<div class="tab-content">
		...
		<form class="tab-pane active" id="st_personal">
			...
			<div class="form-group" id="first_name_group">
				...
				<input type="text" id="first_name" class="form-control setting"
			...
			<div class="form-group" id="twitter_group">
				...
				<div class="input-group">
					...
					<input type="text" id="twitter" class="form-control setting"
					       value="<?php echo $settings['twitter']; ?>">
				</div>
			</div>
			<div class="form-group" id="birthday_group">
				<label for="birthday">Birthday</label>
				<input type="date" id="birthday" name="birth" class="form-control setting"
				       value="<?php echo date('Y-m-d', $settings['birth']) ?>">
			</div>
			<div class="form-group" id="username_group">
				<label for="username">Username</label>
				<input type="text" id="username" class="form-control setting" name="username"
				       value="<?php echo $settings['username'] ?>" pattern="[a-zA-Z0-9]{3,20}">
		...
		<form class="tab-pane" id="st_wall">
			<div class="checkbox">
				<label>
					<input type="checkbox" class="setting"
					       id="wall_enabled" <?php echo ($settings['wall_enabled'] == '1') ? 'checked' : '' ?>> Enable
					wall
				</label>
			</div>
			<div class="checkbox">
				<label>
					<input type="checkbox" class="setting"
					       id="wall_everybody" <?php echo ($settings['wall_everybody'] == '1') ? 'checked' : '' ?>>
					Everybody can see my wall (otherwise - only friends)
				</label>
			</div>
			<hr>
		...
		<form class="tab-pane" id="st_privacy">
			<div class="checkbox">
				<label>
					<input type="checkbox" class="setting"
					       id="messages_everybody" <?php echo ($settings['messages_everybody'] == '1') ? 'checked' : '' ?>>
					Only friends can send messages to me
				</label>
			</div>
			<hr>
		...
		<form class="tab-pane" id="st_additional">
			<div id="color_palette">
				<div class="input-group">
					<label>
						You can choosy the color scheme for header navbar you are prefer:
					</label>
					<?php
					//@TODO Finish it
					?>
					<br>
					<span class="text-muted">Wanna more variations? Just send your wishes at admin@jenott.net!</span>
				</div>
			</div>
			<hr>
...
<script>
	var current;
	if(localStorage.getItem('j_bckg') !== null){
		current = localStorage.getItem('j_bckg');
	} else {
		current = 1;
	}
</script>